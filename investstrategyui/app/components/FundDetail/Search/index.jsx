...
class Search extends Component {
  ...
  componentDidMount() {
    ...
    // todo....
    // 监听滚动
    const typeBoxRes = document.getElementsByClassName('productCenter-search__content')[0]
  ...
  */
  ...
  render() {
    ...
    return (
      <div className="productCenter-search">
        <Input placeholder="请输入产品名称/代码" prefix={<SearchOutlined />} onChange={e => this.handleSearch(e)} onPressEnter={e => this.handleSearch(e)}/>
        <div className="productCenter-search__content" style={{display: isShowContent ? 'block' : 'none'}}>
                  ...
                  <div className="fund" key={list[index].id} onClick={() => this.handleClick(list[index].id)}>
                    <span className="fundName">{list[index].name}</span>
                    {
                      item.map((subItem,subIndex) => {
                        return (
                          <span key={index + subIndex} className="secId" style={{color: subItem.matched ? '#9b7235' : '#666'}}>
                            {subItem.id}
                          </span>
                        )
                      })
