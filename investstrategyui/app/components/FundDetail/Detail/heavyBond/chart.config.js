  ...
  const option = {
    ...
    tooltip: {
      ...
      formatter: function (params) {
        ...
        let htmlStr = `<div><span style="display: inline-block;margin-right: 6px;width: 10px;height: 10px;border-radius: 2px;background: ${params[0].color}"></span>
                        <span>${params[0].name}</span>
                        <span>（${params[0].data.fundId}）</span>
                      </div>`
        htmlStr += `<div style="margin: 6px 0">占净值比：${val !== '--' ? val + '%' : '--'}</div>`
