  ...
  const options = {
    ...
    tooltip: {
      ...
      formatter: (params) => {
        ...
        let htmlStr = '<div style="text-align: left;margin-bottom: 6px;font-weight: 500;">' + date + '</div>'
        for (let i = 0; i < params.length; i++) {
          const val = params[i].value[1]
          if (val) {
            htmlStr += `<div style="margin-bottom: 6px;">
                          <span style="display: inline-block;width: 10px;height: 10px;border-radius: 2px;margin-right: 6px;background: ${params[i].color}"></span>
                          <span style="color: ${params[i].seriesIndex === 0 ? '#dcae73' : '#55adf6'}">${params[i].seriesName}</span>
                          <span>：${val}%</span>
                        </div>`
          } else {
            htmlStr += `<div>
