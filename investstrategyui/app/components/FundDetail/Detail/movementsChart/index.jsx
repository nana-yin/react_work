...
import { MOVEMENTS_TIME_CARD, isEmpty, accMul } from '../../../../util/fund'
// 引入当前页面的样式
import './index.less'
  ...
  initChart = (val) => {
    ...
    if (Object.keys(val).length > 0) { // 有数据
      ...
      const tableData = [
        { key: '1',
          name: fundName,
          rateReturn: fundList.length > 0 ? parseFloat(fundList[fundList.length - 1]).toFixed(2) + '%' : '--',
          rate: !isEmpty(fundDict.rtnRateYearly) ? `${parseFloat(fundDict.rtnRateYearly).toFixed(2)}%` : '--',
        ...
        },
        {
          key: '2',
          name: benchmarkName,
          rateReturn: benchmark.length > 0 ? parseFloat(benchmark[benchmark.length - 1]).toFixed(2) + '%' : '--',
          rate: !isEmpty(benchmarkDict.rtnYearly) ? `${parseFloat(benchmarkDict.rtnYearly).toFixed(2)}%` : '--',
