...
export const getChart = (legendData, seriesData, xData, multiY, dataZoom, dataSet, title, visualMap) => {
  ...
  const options = {
          ...
          for (let i = 0; i < dataSet.length; i++) {
            if (val.seriesName === dataSet[i].secName) {
              ...
              if (dataSet[i].region === '百分号') {
                ...
                htmlStr += '<div style="text-align: left;"><span style="width: 9px;height: 9px;display:inline-block;border-radius: 50%;background-color:' + (Object.keys(visualMap).length > 0 ? '#af8e5d' : val.color) + ';margin-right:6px"></span><span>' + val.seriesName + '：</span>' +
                '<span">' + v + '</span"></div>'
              } else {
                htmlStr += '<div style="text-align: left;"><span style="width: 9px;height: 9px;display:inline-block;border-radius: 50%;background-color:' + (Object.keys(visualMap).length > 0 ? '#9b7235' : val.color) + ';margin-right:6px"></span><span>' + val.seriesName + '：</span>' +
                '<span>' + value + '</span></div>'
              }
              break
            }
    ...
    legend: {
      ...
      bottom: '0%',
      data: legendData,
      selectedMode: legendData.length > 1,
      icon: 'rect',
      itemWidth: 8,
      itemHeight: 8,
      itemGap: 20,
